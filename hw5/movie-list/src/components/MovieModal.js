...
const MovieModal = (movie) => {
    ...
    return (
        <Fragment>
            <Button size="sm" color="info" onClick={toggle}>Get Details</Button>
            <Modal isOpen={modal} toggle={toggle}>
                <ModalHeader toggle={toggle} >{movie.movie.title}</ModalHeader>
                <ModalBody style={modalStyle}>
                    <div style={{ display: "flex", flexDirection: "row", justifyContent: "center" }}>
                        <img src={url2} style={{ width: "70%", height: "auto" }} alt="pic"></img>
                    </div>
                    ...
                    <div style={{ display: "flex", flexDirection: "row", justifyContent: "center" }}>
                        ...
                        <h5 style={{ textTransform: 'uppercase' }}>{movie.movie.original_language}</h5>
                    </div>
                    <div style={{ display: "flex", flexDirection: "row", justifyContent: "center" }}>
                                ...
                                () => {
                                    ...
                                    if (genreName) {
                                        ...
                                        return (
                                            <Fragment>
                                                {genreName.map((g, i) => (
                                                    <h6 key={g} style={{ backgroundColor: color[i], padding: "5px 10px" }}>{g}</h6>
                                                ))}
                                            </Fragment>
                                        );
                                    }
